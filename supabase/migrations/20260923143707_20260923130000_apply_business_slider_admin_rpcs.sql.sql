/*
# Apply Missing Business Slider Admin RPCs

## Summary
The previous Business Slider migration (20260923120000) created the tables,
storage bucket, public RLS policy, and the public RPC `get_active_business_slides`,
but the 10 admin RPCs were never applied to the database. This caused the admin
Business Slider page to show "بارگذاری اسلایدها انجام نشد" because the frontend
calls `admin_list_business_slides` which did not exist.

## Changes
This migration creates the following 11 SECURITY DEFINER functions (all with
JWT role check for 'admin' or 'super_admin'), revokes PUBLIC execute, and grants
execute to `authenticated`:

1. `admin_list_business_slides` — list all slides with layer count
2. `admin_get_business_slide(p_slide_id)` — single slide with full layers
3. `admin_create_business_slide` — create a new slide (24 params)
4. `admin_update_business_slide` — update slide fields (33 params incl. clear flags)
5. `admin_delete_business_slide(p_slide_id)` — delete a slide
6. `admin_duplicate_business_slide(p_slide_id)` — deep copy slide + layers
7. `admin_reorder_business_slides(p_items)` — reorder slides
8. `admin_create_slide_layer` — create a layer on a slide (13 params)
9. `admin_update_slide_layer` — update a layer (17 params incl. clear flags)
10. `admin_delete_slide_layer(p_layer_id)` — delete a layer
11. `admin_reorder_slide_layers(p_slide_id, p_items)` — reorder layers within a slide

## Security
- All functions are SECURITY DEFINER with SET search_path TO public.
- Each function checks `auth.jwt() -> 'raw_app_meta_data' ->> 'role'` (with
  fallback to `auth.jwt() ->> 'role'`) and returns `{'success': false, 'error': 'unauthorized'}`
  if the role is not 'admin' or 'super_admin'.
- EXECUTE is revoked from PUBLIC and granted only to `authenticated` for all
  admin functions.
...
- No changes to `get_active_business_slides` (already exists).
*/
