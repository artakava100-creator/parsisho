/*
# Apply Missing Business Slider Admin RPCs (v2)

## Summary
Two prior migrations attempted to create the Business Slider admin RPCs but
the functions were never actually applied to the database. Only the public
RPC `get_active_business_slides` exists. The admin Business Slider page
shows "بارگذاری اسلایدها انجام نشد" because `admin_list_business_slides`
does not exist.

## Changes
Creates 11 SECURITY DEFINER functions for admin Business Slider management.
Each function checks JWT role for 'admin' or 'super_admin', revokes PUBLIC
execute, and grants execute to authenticated.
