...
export function SpecialCard({ title, description, icon, imageUrl, destinationUrl, preview }: SpecialCardProps) {
    ...
    <>
      ...
      <div className="relative flex-1 flex flex-col items-center justify-center p-3 sm:p-4 overflow-hidden">
        {hasImage ? (
          ...
          <>
            <div className="w-10 h-10 sm:w-12 sm:h-12 lg:w-14 lg:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center shrink-0 bg-accent-50 text-accent-700 group-hover:bg-accent-100 group-hover:scale-110 transition-all duration-normal">
      ...
      {!hasImage && (
        <div className="flex items-center justify-center pb-2 sm:pb-3">
          <ArrowLeft className="w-3.5 h-3.5 text-neutral-300 group-hover:text-accent-500 transition-colors" />
        </div>
      )}
    </>
  );
  ...
  const baseClasses = cn(
    ...
    'hover:border-accent-300/70 hover:shadow-lg hover:shadow-accent-900/5 hover:-translate-y-1',
    'active:scale-[0.98]',
