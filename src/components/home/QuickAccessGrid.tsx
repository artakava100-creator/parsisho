...
export function QuickAccessGrid() {
  ...
  return (
    <div className="grid grid-cols-2 gap-2 sm:gap-3 sm:flex sm:flex-row sm:overflow-visible">
      {items.map((item, idx) => {
        ...
        return (
          <Link
            ...
            className={cn(
              'group flex items-center gap-2.5 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl bg-white border border-neutral-200/80',
          ...
          >
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center shrink-0 bg-primary-50 text-primary-700 group-hover:bg-primary-100 transition-colors">
              <Icon className="w-4 h-4 sm:w-5 sm:h-5" />
            </div>
            <h3 className="text-xs sm:text-sm font-bold text-neutral-800 whitespace-nowrap">
              {item.label}
            </h3>
